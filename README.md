...
- `SmallCancellation/PlanarCancellation.lean` turns each nested free-reduction tree into absolute source positions for its cancelled letter pairs and retained letters. Lean proves that the pairing count matches the tree count, every cancellation joins inverse source letters, each interval is forward and within the source word, and the intervals are noncrossing. The retained occurrences spell the reduced output in order and point to those same letters in the source. The module does not yet identify edges/vertices or build the relator faces into a planar map.
- `SmallCancellation/PlanarBoundarySeed.lean` applies that trace to a minimum-area relator certificate's literal conjugate-relator boundary. It packages the cancellation count, noncrossing inverse-letter pairs, source bounds, and the ordered surviving boundary occurrences. This is explicit indexed boundary data, not yet a planar van Kampen diagram or a proof of Greendlinger's lemma.
