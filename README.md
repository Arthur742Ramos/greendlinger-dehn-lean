...
- `SmallCancellation/PlanarCancellation.lean` turns each nested free-reduction tree into absolute source positions for its cancelled letter pairs. Lean proves that the pairing count matches the tree count, each interval is forward and within the source word, and the intervals are noncrossing. The module does not yet identify edges/vertices or build the relator faces into a planar map.
