...
`SmallCancellation/Curvature.lean` formalizes the finite Gauss--Bonnet ledger and angle count. Its accounting theorem derives total face curvature 2 from Euler's equation and aggregate angle/side incidence counts; the local face theorem then forces a shell with at most three internal arcs. It does not derive those accounting identities or local face bounds from an actual disk map. The release-blocking work remains (1) constructing a reduced planar diagram from a nullity certificate, (2) deriving disk Euler/angle identities and local incidence bounds from an actual combinatorial disk model, and (3) extracting the relator subword from the shell.
